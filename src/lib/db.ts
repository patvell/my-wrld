...
// Seed data from Supabase export — only runs if table is empty
const count = (db.prepare('SELECT COUNT(*) as c FROM flights').get() as { c: number }).c;
if (count === 0) {
  const seedData = [
  ...
  const insert = db.prepare(`
    INSERT OR IGNORE INTO flights (id, created_at, origin_code, origin_city, destination_code, destination_city, departure_time, arrival_time, status, type, flight_number, confirmed_at, user_id)
    VALUES (@id, @created_at, @origin_code, @origin_city, @destination_code, @destination_city, @departure_time, @arrival_time, @status, @type, @flight_number, @confirmed_at, @user_id)
  `);

  const insertMany = db.transaction((flights: typeof seedData) => {
