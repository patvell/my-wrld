...
- **Server-side landing detection:** `/api/cron/sync-status` syncs flights inside their live window with AeroAPI and marks them `completed`/`cancelled`, so trips move to History when the aircraft actually lands — even with the app closed. Set `CRON_SECRET` to restrict the route (sent as a Bearer token by whichever scheduler calls it).
  - `vercel.json` schedules it once a day — that's the finest grain Vercel's free Hobby plan allows for cron jobs. Fine as a nightly safety-net sweep, but not "as it lands."
  - For near-real-time detection at no cost, `.github/workflows/sync-status.yml` calls the same endpoint every 10 minutes using GitHub's own scheduler (unaffected by Vercel's plan limits). Set two repo secrets under **Settings → Secrets and variables → Actions**: `APP_URL` (your deployed origin, e.g. `https://my-wrld.vercel.app`) and `CRON_SECRET` (must match the Vercel env var). On a Vercel Pro plan you can instead tighten `vercel.json`'s schedule and drop the workflow.
